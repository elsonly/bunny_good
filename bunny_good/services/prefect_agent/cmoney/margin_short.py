from prefect import flow, task, get_run_logger
from prefect.task_runners import SequentialTaskRunner
import xlwings as xw
import pandas as pd
from typing import Dict, List
from pathlib import Path

from bunny_good.database.data_manager import DataManager
from bunny_good.services.prefect_agent.utils import flow_error_handle


def get_workbook_path() -> Path:
    try:
        name = Path(__file__).absolute().name.replace(".py", "")
        wb_path = Path(__file__).parent.joinpath(f"docs/{name}.xlsm")
    except:
        wb_path = (
            Path()
            .absolute()
            .joinpath("bunny_good/services/prefect_agent/cmoney/docs/margin_short.xlsm")
        )
    return wb_path


@task(
    name="task-margin_short-update_workbook",
    retries=3,
    retry_delay_seconds=3,
)
def update_workbook(
    start_date: pd.Timestamp = None,
    end_date: pd.Timestamp = None,
    periods: List[pd.Timestamp] = [],
    validate_date: bool = False,
):
    def get_validate_date(tdate: pd.Timestamp) -> pd.Timestamp:
        if not validate_date or not periods:
            return tdate
        if tdate < pd.to_datetime("1997-02-01"):
            return tdate

        while tdate <= periods[-1]:
            if tdate in periods:
                return tdate
            tdate += pd.offsets.Day()
        return None

    logger = get_run_logger()
    items = {
        "資買": "日融資券排行",
        "資賣": "日融資券排行",
        "資現償": "日融資券排行",
        "資餘": "日融資券排行",
        "資增減": "日融資券排行",
        "資限": "日融資券排行",
        "券買": "日融資券排行",
        "券賣": "日融資券排行",
        "券賣金額(千)": "日融資券排行",
        "券現償": "日融資券排行",
        "券餘": "日融資券排行",
        "券增減": "日融資券排行",
        "資券相抵": "日融資券排行",
        "券資比": "日融資券排行",
        "資使用率": "日融資券排行",
        "券使用率": "日融資券排行",
        "當沖比率": "日融資券排行",
        "資券註": "日融資券排行",
        "借券賣出": "日融資券排行",
        "借券賣出金額(千)": "日融資券排行",
        "借券賣出還券": "日融資券排行",
        "借券賣出調整": "日融資券排行",
        "借券賣出庫存異動": "日融資券排行",
        "借券賣出餘額": "日融資券排行",
        "借券可使用額度": "日融資券排行",
        "借券系統當日借券": "日融資券排行",
        "借券系統當日還券": "日融資券排行",
        "借券系統借券餘額異動": "日融資券排行",
        "借券系統借券餘額": "日融資券排行",
        "借券系統借券餘額市值": "日融資券排行",
        "證商營業處所當日借券": "日融資券排行",
        "證商營業處所當日還券": "日融資券排行",
        "證商營業處所借券餘額異動": "日融資券排行",
        "證商營業處所借券餘額": "日融資券排行",
        "證商營業處所借券餘額市值": "日融資券排行",
        "借貸專戶當日借券": "日融資券排行",
        "借貸專戶當日還券": "日融資券排行",
        "借貸專戶借券餘額異動": "日融資券排行",
        "借貸專戶借券餘額": "日融資券排行",
        "借貸專戶借券餘額市值": "日融資券排行",
        "融資成本(推估)": "日融資券排行",
        "融券成本(推估)": "日融資券排行",
        "融資維持率(%)": "日融資券排行",
        "融券維持率(%)": "日融資券排行",
        "整體維持率(%)": "日融資券排行",
    }
    wb_path = get_workbook_path()
    logger.info(wb_path)

    with xw.App() as app:
        wb = xw.Book(wb_path)
        sh = wb.sheets("工作表1")

        logger.info("clear contents...")
        rng = sh.range("A3:CNZ10000")
        rng.delete()

        logger.info("update request items...")
        idx = 3
        for item, table in items.items():
            req_date = get_validate_date(start_date)
            while req_date and req_date <= end_date:
                sh.range(f"A{idx}").value = f"{table}.{item}"
                sh.range(f"B{idx}").value = req_date.strftime("%Y%m%d")
                req_date = get_validate_date(req_date + pd.offsets.Day())

                idx += 1

        logger.info("update data...")
        wb.macro("工作表1.CM_Renew")()

        logger.info("save and close workbook")
        wb.save()
        wb.close()


@task(name="task-margin_short-process_data")
def process_data() -> Dict[str, pd.DataFrame]:
    collection = {}
    wb_path = get_workbook_path()
    df = pd.read_excel(wb_path, 0, dtype=str)
    indexes = df.iloc[1:, 1].unique()
    indexes.sort()
    df.set_index("基準日:最近一日", inplace=True)

    col_map = {
        "資買": "margin_buy",
        "資賣": "margin_sell",
        "資現償": "margin_redemption",
        "資餘": "margin_balance",
        "資增減": "margin_balance_change",
        "資限": "margin_limit",
        "券買": "short_buy",
        "券賣": "short_sell",
        "券賣金額(千)": "short_sell_amount",
        "券現償": "short_sell_redemption",
        "券餘": "short_balance",
        "券增減": "short_balance_change",
        "資券相抵": "margin_short_offset",
        "券資比": "short_margin_ratio",
        "資使用率": "margin_utilization_rate",
        "券使用率": "short_utilization_rate",
        "當沖比率": "day_trading_ratio",
        "資券註": "margin_short_remarks",
        "借券賣出": "sb_short_sell",
        "借券賣出金額(千)": "sb_short_sell_amount",
        "借券賣出還券": "sb_short_sell_return",
        "借券賣出調整": "sb_short_sell_adjustment",
        "借券賣出庫存異動": "sb_short_sell_inventory_change",
        "借券賣出餘額": "sb_short_sell_balance",
        "借券可使用額度": "sb_short_sell_available_limit",
        "借券系統當日借券": "sb_system_daily_borrowing",
        "借券系統當日還券": "sb_system_daily_return",
        "借券系統借券餘額異動": "sb_system_balance_change",
        "借券系統借券餘額": "sb_system_balance",
        "借券系統借券餘額市值": "sb_system_balance_market_value",
        "證商營業處所當日借券": "sb_broker_daily_borrowing",
        "證商營業處所當日還券": "sb_broker_daily_return",
        "證商營業處所借券餘額異動": "sb_broker_balance_change",
        "證商營業處所借券餘額": "sb_broker_balance",
        "證商營業處所借券餘額市值": "sb_broker_balance_market_value",
        "借貸專戶當日借券": "sb_margin_account_daily_borrowing",
        "借貸專戶當日還券": "sb_margin_account_daily_return",
        "借貸專戶借券餘額異動": "sb_margin_account_balance_change",
        "借貸專戶借券餘額": "sb_margin_account_balance",
        "借貸專戶借券餘額市值": "sb_margin_account_balance_market_value",
        "融資成本(推估)": "estimated_margin_cost",
        "融券成本(推估)": "estimated_short_sell_cost",
        "融資維持率(%)": "margin_maintenance_rate",
        "融券維持率(%)": "short_sell_maintenance_rate",
        "整體維持率(%)": "overall_maintenance_rate",
    }
    item_start_idx = 8
    items = df.iloc[1:, 3].str[item_start_idx:].unique()
    tmp_rows = {}
    for item in items:
        tmp_rows[item] = df.loc[df.iloc[:, 3].str[item_start_idx:] == item]

    for code in df.columns[4:]:
        tmp = pd.DataFrame(index=indexes, columns=items)
        for item in items:
            tmp.loc[:, item] = tmp_rows[item][code]

        tmp.dropna(how="all", axis=0, inplace=True)
        tmp.rename(columns=col_map, inplace=True)
        if tmp.empty:
            continue
        tmp["tdate"] = pd.to_datetime(tmp.index)
        tmp["code"] = code
        for col in tmp.columns:
            tmp.loc[pd.isnull(tmp[col]), col] = None

        collection[code] = tmp

    return collection


@task(name="task-margin_short-save2db", log_prints=True)
def save2db(collection: Dict[str, pd.DataFrame]):
    dm = DataManager(verbose=False)
    for code, df in collection.items():
        dm.save(
            "cmoney.margin_short",
            df,
            method="timeseries",
            time_col="tdate",
            conditions={"code": code},
        )


@task(name="task-margin_short-get_last_date", log_prints=True)
def get_last_date() -> pd.Timestamp:
    dm = DataManager(verbose=False)
    last_date = dm.get_max_datetime("cmoney.margin_short", {}, "tdate")
    if last_date is None:
        return pd.to_datetime("2000-01-04")
    else:
        return pd.to_datetime(last_date)


@task(name="task-margin_short-get_trading_dates", log_prints=True)
def get_trading_dates() -> List[pd.Timestamp]:
    dm = DataManager(verbose=False)
    tdates = [pd.to_datetime(x) for x in dm.get_twse_trading_dates()]
    return tdates


@flow(
    retries=2,
    retry_delay_seconds=30,
    task_runner=SequentialTaskRunner(),
    on_failure=flow_error_handle,
)
def flow_margin_short_history():
    logger = get_run_logger()
    today = pd.Timestamp.today()
    start_date = get_last_date() + pd.offsets.Day()
    trading_dates = get_trading_dates()
    while start_date <= today:
        end_date = start_date + pd.offsets.MonthEnd()
        if end_date >= today:
            end_date = today
        logger.info(f"{start_date} ~ {end_date}")
        update_workbook(start_date, end_date, periods=trading_dates, validate_date=True)
        collections = process_data()
        save2db(collections)
        start_date += pd.offsets.MonthBegin()


@flow(
    retries=2,
    retry_delay_seconds=30,
    timeout_seconds=1200,
    task_runner=SequentialTaskRunner(),
    on_failure=[flow_error_handle],
    on_crashed=[flow_error_handle],
)
def flow_margin_short():
    logger = get_run_logger()
    end_date = pd.Timestamp.today()
    start_date = end_date - 5 * pd.offsets.BDay()
    logger.info(f"{start_date} ~ {end_date}")
    update_workbook(start_date, end_date)
    collections = process_data()
    save2db(collections)
